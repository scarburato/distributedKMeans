...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
				width=.8\linewidth,
				title={Example of syntethic clusters ($k=4$)}
			]
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Procedure{ComputeAverage}{$B$} \Comment{The same as \ref{computeAvg}}
			\State \Return $\dfrac{1}{k}\sum_{i = 1}^{k} p_i$
		\EndProcedure
		
		\vspace{.25cm}
		
		\Procedure{MergeAverage}{$B_a$, $B_b$} \Comment{The same as 
		\ref{mergeAvg}}
...
\begin{figure}[H]
	...
	\begin{axis}[
		...
		\addplot table [x=size, y=time, col sep=comma] {../bench/k3d12bis.csv};
		\addplot table [x=size, y=time, col sep=comma] {../bench/k3d12.min.csv};
		%\addplot +[mark=none] coordinates {(3500000, 300) (3500000, 20000)};
	\end{axis}
\end{tikzpicture}
